...
\begin{align*}
  \ingraph{v} &= \SetOf{\e{=}(u, v', p, v) \mapsto G(\e) \SuchThat{G(\e) \in \SetOf{\Plus, \Minus}}} \\
  \outedges{v}{p} &= \SetOf{\e{=}(u, v, p, v') \SuchThat{G(\e) = \Plus}} \\
  % \children{v} &= \SetOf{v' \SuchThat{\exists \e{=}(u, v, p, v') \land G(\e) = \Plus}} \\
...
\newcommand{\figureDecompositionDefDecompComponents}{%
  \Set[NP] &= \SetOf{\edecomp{(u, v, p, v')} \SuchThat{|\parents{v'}| = 0}} \\
  \Set[MP] &= \SetOf{\edecomp{(u, v, p, v')} \SuchThat{|\parents{v'}| > 1}} \\
  \Set[U] &= \SetOf{\edecomp{(u, v, p, v')} \SuchThat{|\parents{v'}| = 1 \land v' = \min{\ancestors{v'}}}}%
}
...
\begin{align*}
  ...
  \edecomp{\e{=}(u, v, p, (u', \ExpApp))} &= \eApp{\ingraph{v'}}{e_\AppFun}{e_\AppArg} \\
...
\[
  \begin{array}{l}
    ...
    \left\{
      \begin{array}{ll}
        \conflictHole{\edecomp{\e_1}, \ldots, \edecomp{\e_n}} & \outedges{v'}{p'} = \SetOf{\e_1, \ldots, \e_n} \land n > 1 \\
        \multiVertex{\e'} & \outedges{v'}{p'} = \SetOf{\e'} \land \e' = (u_{\e'}, v_{\e'}, p_{\e'}, v_{\e'}') \\
            & \phantom{\outedges{v'}{p'} = \SetOf{\e'}} \land |\parents{v_{\e'}'}| > 1 \\
        \cycleVertex{\e'} & \outedges{v'}{p'} = \SetOf{\e'} \land \e' = (u_{\e'}, v_{\e'}, p_{\e'}, v_{\e'}') \\
            & \phantom{\outedges{v'}{p'} = \SetOf{\e'}} \land |\parents{v_{\e'}'}| = 1 \\
            & \phantom{\outedges{v'}{p'} = \SetOf{\e'}} \land v_{\e'}' = \min{\ancestors{v_{\e'}'}} \\
        \hole & \outedges{v'}{p'} = \varnothing \\
...
\[
  \begin{array}{l}
    ...
    \left\{
      \begin{array}{ll}
        \conflictHole{\pdecomp{\e_1}, \ldots, \pdecomp{\e_n}} & \outedges{v'}{p'} = \SetOf{\e_1, \ldots, \e_n} \land n > 1 \\
        \multiVertex{\e'} & \outedges{v'}{p'} = \SetOf{\e'} \land \e' = (u_{\e'}, v_{\e'}, p_{\e'}, v_{\e'}') \\
            & \phantom{\outedges{v'}{p'} = \SetOf{\e'}} \land |\parents{v_{\e'}'}| > 1 \\
        \cycleVertex{\e'} & \outedges{v'}{p'} = \SetOf{\e'} \land \e' = (u_{\e'}, v_{\e'}, p_{\e'}, v_{\e'}') \\
            & \phantom{\outedges{v'}{p'} = \SetOf{\e'}} \land |\parents{v_{\e'}'}| = 1 \\
...
\[
  \begin{array}{l}
    ...
    \left\{
      \begin{array}{ll}
        \conflictHole{\tdecomp{\e_1}, \ldots, \tdecomp{\e_n}} & \outedges{v'}{p'} = \SetOf{\e_1, \ldots, \e_n} \land n > 1 \\
        \multiVertex{\e'} & \outedges{v'}{p'} = \SetOf{\e'} \land \e' = (u_{\e'}, v_{\e'}, p_{\e'}, v_{\e'}') \\
            & \phantom{\outedges{v'}{p'} = \SetOf{\e'}} \land |\parents{v_{\e'}'}| > 1 \\
        \cycleVertex{\e'} & \outedges{v'}{p'} = \SetOf{\e'} \land \e' = (u_{\e'}, v_{\e'}, p_{\e'}, v_{\e'}') \\
            & \phantom{\outedges{v'}{p'} = \SetOf{\e'}} \land |\parents{v_{\e'}'}| = 1 \\
