...
\path (-5em,0cm) node (node/Move/bx) [gray,anchor=base west] {{\textbf{\small \ref*{fig:Move:b}}}};
\path (0cm,0cm) node (node/DifferentParts/a) [anchor=base] {\strut};
\path [draw,->,alice step] (node/Move/bx) to (-1.5em, 0cm |- node/DifferentParts/a);
\end{tikzpicture}
}%
...
%%%%%%%%
\hfill{}
\caption{(a) Alice fills the hole in the multiplication with \li{u}. (b) Bob renames \li{y} to \li{v}. (c) These  edits to the same expression commute, addressing the \textbf{granularity problem}.}%
...
%%%%%%%%
\hfill{}
\caption{(a) Alice renames \li{u} to \li{w}. (b) Bob relocates the multiplication to the left side. (c) These edits commute without conflict or duplication, addressing the \textbf{relocation modification problem}.}%
\Description {This figure describes users editing nested parts of code}
