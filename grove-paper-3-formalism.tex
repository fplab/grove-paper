...
This section
  gives the syntax of our source language,
  discusses the internal representations of graphs and terms,
  presents a commutative graph edit semantics,
  and introduces the user action semantics.

% We begin in Sec.~\ref{sub:Syntax} with the syntax,
% then discuss the internal graph representation in Sec.~\ref{sub:Convergent Graphs},
% ...
...
The syntax of terms is sorted into expressions, types, and patterns.
Expressions include
  variables $\eVar{G}{x}$,
...
conflict holes $\conflictHole{t_i}_{i < n}$,
and empty holes $\hole$.
Note that for conflict holes, it must be the case that $n \geq 2$.
...
and $\Sigma = \SetOf{\bot, \Plus, \Minus}$ the
  \emph{edge states} equipped with a total ordering $\bot \sqsubset \Plus \sqsubset \Minus$
  that forms a lattice over $\Sigma$.
...
A vertex $v = (u, k) \in \V$ is an instance of constructor $k \in \K$ identified by $u \in \U$.
Vertices correspond to \emph{constructable terms}---terms that are not references or holes.
